...
if ($method === 'GET') {
    ...
    if (isset($_GET["keys"])) {
        // get Tickets columns
        $query = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'RegisteredTickets' AND TABLE_SCHEMA = '$db_name'";
    ...
    if (!isset($_GET["keys"]) || (isset($_GET["keys"]) && str_contains($_GET["keys"],"bets"))) {
        ...
        $query = "SELECT * FROM RegisteredTicketBets";
        $result = mysqli_query($conn, $query);
        while($row = mysqli_fetch_assoc($result)) {
	...
	$query = "SELECT ". $columns ." FROM RegisteredTickets WHERE ". $filter;
	$result = mysqli_query($conn, $query);
	while($row = mysqli_fetch_assoc($result)) {
	    if (count($users) > 0) {
    ...
    if ($_GET["mode"] == "update") {
        ...
        if ($newData["id"]) {
            ...
            $query = "DELETE FROM RegisteredTicketBets WHERE ticketId = '".$newData["id"]."'";   
            $result = mysqli_query($conn, $query);
             
            // reset ticket
            $query = "UPDATE RegisteredTickets SET odds='0', ticketValue='10', wins='0' WHERE id = '".$newData["id"]."'";
            $result = mysqli_query($conn, $query);
        }
    }
    else if ($_GET["mode"] == "increase") {
        // check if it exists
        $query = "SELECT * FROM RegisteredTicketBets WHERE betID = '".$newData["betId"]."' AND ticketId = '".$newData["ticketId"]."';";
        $result = mysqli_query($conn, $query);
        if (mysqli_num_rows($result) == 0) {
            if ($newData["betId"] && $newData["ticketId"]) {
                $query = "INSERT INTO RegisteredTicketBets (betId, ticketId) VALUES ('".$newData["betId"]."', '".$newData["ticketId"]."');";
                $result = mysqli_query($conn, $query);
            }
        }
    }
        ...
        if ($newData["betId"] && $newData["ticketId"]) {
            $query = "DELETE FROM RegisteredTicketBets WHERE betID = '".$newData["betId"]."' AND ticketId = '".$newData["ticketId"]."';";
            $result = mysqli_query($conn, $query);
        }
    }
}
